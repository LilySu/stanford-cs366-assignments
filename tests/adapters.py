...
def run_linear(
    ...
    # 2. Instantiate the custom Linear layer
    # Use the input tensor's device/dtype to prevent errors
    model = Linear(
        in_features=d_in, 
        out_features=d_out, 
    ...
    # The test suite gives weights as (d_out, d_in).
    # Linear class expects W to be (d_in, d_out).
    # TRANSPOSE (.T) the weights to match your shape.
    weights_for_loading = weights.T

    # 4. Load the weights using load_state_dict
    # Create a dictionary matching the parameter name "W" in your class
    state_dict = {"W": weights_for_loading}
    
    # strict=True ensures the names match exactly (useful for debugging)
    model.load_state_dict(state_dict, strict=True)

    # 5. Run the model
    # Use no_grad() because we are just testing inference, not training
    with torch.no_grad():
...
def run_embedding(
    ...
    # Instantiate custom Embedding class
    # Pass the device/dtype from the input weights to ensure compatibility
    embed = Embedding(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
    
    # Manually load the provided test weights into module
    # We must wrap the tensor in nn.Parameter as class expects
...
def run_multihead_self_attention(
    ...
    # Your Linear class uses shape (In, Out) and x @ W.
    # We must transpose (.T) the weights.
    ...
    # 3. Run Forward (No RoPE as requested by docstring)
