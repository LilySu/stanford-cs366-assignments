...
class TransformerBlockNoNorm(nn.Module):
    def __init__(self, d_model: int, num_heads: int, d_ff: int, max_seq_len: int, theta: float, device=None, dtype=None):
        ...
        self.ffn_w1 = Linear(d_model, d_ff, device=device, dtype=dtype)
        self.ffn_w2 = Linear(d_ff, d_model, device=device, dtype=dtype)
        self.ffn_w3 = Linear(d_model, d_ff, device=device, dtype=dtype)
...
class TransformerLMNoNorm(nn.Module):
    def __init__(self, vocab_size, context_length, d_model, num_layers, num_heads, d_ff, rope_theta, device=None, dtype=None):
        ...
        self.layers = nn.ModuleList([
            TransformerBlockNoNorm(d_model, num_heads, d_ff, context_length, rope_theta, device, dtype)
...
def reinit_weights_gpt_style(model, std=0.02):
    """
    CRITICAL FIX: Re-initialize weights to be small (std=0.02).
    Without RMSNorm, the default std=1.0 initialization causes 
    immediate activation explosion (NaN) in the forward pass.
    """
    print(f"Applying Manual Init (std={std}) for No-Norm Stability...")
    for name, p in model.named_parameters():
        if 'W' in name or 'weight' in name:
            # Skip 1D parameters (biases) if any
            if p.dim() >= 2:
...
def get_args():
    parser = argparse.ArgumentParser(description="Transformer No-Norm Experiment")
    ...
    parser.add_argument("--wandb_project", type=str, default="transformer-no-norm")
...
def main():
    ...
    if args.mode == "sample":
        if args.checkpoint and os.path.exists(args.checkpoint):
            ...
            model = TransformerLMNoNorm(device=device, **model_config)
    ...
    # 1. Instantiate No-Norm Model
    model = TransformerLMNoNorm(device=device, **model_config)
    
    # 2. APPLY FIX: Re-initialize weights
    reinit_weights_gpt_style(model, std=0.02)
    ...
    print(f"Starting NO-NORM training on {device}...")
    ...
    while iter_num < args.max_iters:
        ...
        if iter_num > 0 and iter_num % args.save_interval == 0:
            ckpt_path = os.path.join(args.out_dir, "ckpt_no_norm.pt")
