...
from modules.transformers import (
    ...
    # RotaryPositionalEmbedding, <--- We won't use this!
    AdamW, 
...
# ==============================================================================
# NoPE (No Positional Embeddings) OVERRIDES
# ==============================================================================
...
class TransformerBlockNoPE(nn.Module):
    def __init__(self, d_model, num_heads, d_ff, max_seq_len, theta, device=None, dtype=None):
        ...
        # REMOVED: self.rope = RotaryPositionalEmbedding(...)
        # We just don't initialize it.

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # REMOVED: positions = torch.arange(...)
        
        # Attention Sublayer
        resid = x
        x = self.ln1(x)
        
        # KEY CHANGE: Pass rope_module=None, token_positions=None
        # The MultiHeadSelfAttention logic will skip the rotation step.
        x = self.attn(x, rope_module=None, token_positions=None)
...
class TransformerLMNoPE(nn.Module):
    def __init__(self, vocab_size, context_length, d_model, num_layers, num_heads, d_ff, rope_theta, device=None, dtype=None):
        ...
        self.layers = nn.ModuleList([
            # Use the NoPE Block
            TransformerBlockNoPE(d_model, num_heads, d_ff, context_length, rope_theta, device, dtype)
...
def get_args():
    parser = argparse.ArgumentParser(description="Transformer NoPE Experiment")
    ...
    parser.add_argument("--wandb_project", type=str, default="transformer-nope")
...
def main():
    ...
    if args.mode == "sample":
        if args.checkpoint and os.path.exists(args.checkpoint):
            ...
            model = TransformerLMNoPE(device=device, **model_config)
    ...
    print(f"Starting NoPE training on {device}...")
    ...
    while iter_num < args.max_iters:
        ...
        if iter_num > 0 and iter_num % args.save_interval == 0:
            ckpt_path = os.path.join(args.out_dir, "ckpt_nope.pt")
