...
from modules.transformers import (
    Linear, 
    Embedding, 
    RMSNorm,     # We need RMSNorm this time!
...
class TransformerBlockPostNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 1. Calculate Attention (on x, NOT normed x)
        resid = x
        attn_out = self.attn(x, rope_module=self.rope, token_positions=positions)
        # 2. Add Residual
        x = resid + attn_out
        # 3. Apply Norm AFTER Addition
        x = self.ln1(x)
        
        # --- FFN SUBLAYER (POST-NORM) ---
        resid = x
        # 1. FFN (SwiGLU)
        gate = self.ffn_w1(x)
        up = self.ffn_w3(x)
...
class TransformerLMPostNorm(nn.Module):
    def __init__(self, vocab_size, context_length, d_model, num_layers, num_heads, d_ff, rope_theta, device=None, dtype=None):
        ...
        self.layers = nn.ModuleList([
            TransformerBlockPostNorm(d_model, num_heads, d_ff, context_length, rope_theta, device, dtype)
            for _ in range(num_layers)
        ])
        
        # NOTE: In pure Post-Norm, the output of the last block is ALREADY normalized.
        # We generally do NOT need a final RMSNorm layer here.
        # self.ln_final = RMSNorm(...) <--- Removed
...
def get_args():
    parser = argparse.ArgumentParser(description="Transformer Post-Norm Experiment")
    ...
    parser.add_argument("--wandb_project", type=str, default="transformer-post-norm")
...
def main():
    ...
    if args.mode == "sample":
        if args.checkpoint and os.path.exists(args.checkpoint):
            ...
            model = TransformerLMPostNorm(device=device, **model_config)
    ...
    print(f"Starting POST-NORM training on {device}...")
    ...
    while iter_num < args.max_iters:
        ...
        if iter_num > 0 and iter_num % args.save_interval == 0:
            ckpt_path = os.path.join(args.out_dir, "ckpt_post_norm.pt")
